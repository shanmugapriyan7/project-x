...
const [c,setC]=useState(0)
    const handle = () => {
        axios.get("https://jsonplaceholder.typicode.com/users")
          .then(response => {
            for(let i=0;i<200;i++){
            setUser(response.data[i]?.email);
            setName(response.data[i]?.lng);
            if (response.data[i]?.email === input && response.data[i]?.email === input2) {
                setC(1);
              }
              else if(response.data[i]?.email === input){
                setC(2);
              }
              else if(response.data[i]?.email === input2){
                setC(3);
              }
            }
  ...
  const handleClick1 = () => {
    ...
    alert("Successfully Logged in!");
      ...
      /*   setLoading(true); // Show backdrop and progress indicator
        setTimeout(() => {
            setLoading(false); // Hide backdrop and progress indicator after some time
            navigate("/#");
    ...
    return (
        <div>
            <header className="header43">
                <h1 className="website-name43">JobSpark</h1>
                <button onClick={handleClick} className="signup-button-header43">Sign Up</button>
            </header>

        

            <div className="hole43">
                <div className="container43">
                    <div className="form-container43 signup-container43">
                        <form onSubmit={handleSubmit} className="form43">
                            <div className="login-text43">Login</div>
              ...
              <div className="field-login-43">
                            ...
                            <a href="#" className="forgotpassword-43">Forgot Password?</a>
                            <button onClick={handle} className="button43">Login</button>
                        </form>
                    </div>
                    <div>
                        <img src="https://png.pngtree.com/png-vector/20220330/ourmid/pngtree-online-recruitment-concept-with-a-businessman-holds-big-magnifier-searching-for-png-image_4520229.png" className="login-image43"></img>
                    </div>
                </div>
            </div>
            {/* Backdrop and CircularProgress components */}
           {/*  <Backdrop
                sx={{ color: '#fff', zIndex: (theme) => theme.zIndex.drawer + 1 }}
                open={loading} // Controlled by the loading state
                onClick={() => { }} // Prevent backdrop from closing on click
            >
